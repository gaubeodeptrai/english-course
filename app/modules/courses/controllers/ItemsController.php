...
use app\modules\courses\models\Item;
...
class ItemsController extends Controller
{
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                $fileInstanse = UploadedFile::getInstance($model, 'video');
                
                $model->video = Upload::file($fileInstanse, 'videos', false);
                
                    //$this->flash('success', Yii::t('easyii/help', 'Đã upload thành công'));
                ...
                if (isset($_FILES) && $this->module->settings['itemThumb']) {
                    ...
                    if ($model->image && $model->validate(['image'])) {
                        $model->image = Image::upload($model->image, 'courses');
                    } else {
                        $model->image = '';
                    }
                }
                if ($model->save()) {
                    $this->flash('success', Yii::t('easyii/courses', 'Item created'));
            ...
            if(Yii::$app->request->isAjax){
                ...
                {    
                    $model->video = Upload::file($fileInstanse, 'videos', false);
                        ...
                        $model->video = $model->oldAttributes['video'];
                    }
                if (isset($_FILES) && $this->module->settings['itemThumb']) {
                    $model->image = UploadedFile::getInstance($model, 'image');
                    if ($model->image && $model->validate(['image'])) {
                ...
                if ($model->save()) {
                    $this->flash('success', Yii::t('easyii/courses', 'Item updated'));
                    return $this->redirect(['/admin/'.$this->module->id.'/items/edit', 'id' => $model->primaryKey]);
                } else {
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('easyii/courses', 'Item deleted'));
    }
    ...
    private function generateForm($fields, $data = null)
    {
        ...
        foreach($fields as $field)
        {
            ...
            elseif ($field->type === 'select') {
                $options = ['' => Yii::t('easyii/courses', 'Select')];
