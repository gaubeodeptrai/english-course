...
class Guestbook extends \yii\easyii\components\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            ['reCaptcha', ReCaptchaValidator::className(), 'on' => 'send', 'when' => function(){
                return Yii::$app->getModule('admin')->activeModules['comment']->settings['enableCaptcha'];
            }],
        ];
    }
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            if($insert){
                ...
                $this->time = time();
                $this->new = 1;
                $this->status = Yii::$app->getModule('admin')->activeModules['comment']->settings['preModerate'] ? self::STATUS_OFF : self::STATUS_ON;
            }
            return true;
        } else {
            return false;
    ...
    public function attributeLabels()
    {
        return [
            ...
            'answer' => Yii::t('easyii/comment', 'Answer'),
